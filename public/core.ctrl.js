...
(function () {
    ...
    let app = angular.module('edhubJobsApp');

    app.controller('CoreCtrl', ['$rootScope', '$scope', '$mdSidenav',
        '$mdDialog', '$timeout', 'edhubAuthService', '$location',
        CoreClass
    ]);

    app.controller('HackFindCtrl', ['$rootScope', '$scope', '$mdSidenav',
        '$mdDialog', '$timeout', 'edhubAuthService', '$location', '$http',
        HackFindClass]);

    function HackFindClass($rootScope, $scope, $mdSidenav, $mdDialog,
                           $timeout, edhubAuthService, $location, $http) {
        const vm = this;
        ...
        vm.hackFindMessage = "hello from Hack Find Ctrl Class ^_^/";

        vm.joinPost = function () {
            $http.post(
                "https://hackersconnect.herokuapp.com/api/hackers/",
                vm.joinDataModel
            ).then(function (res) {
                console.log("The response from the body: ", res);
            });
            console.log("jha - just sent the post request");
        }

        vm.goToHomeView = function () {
            $location.path('/');
        }
    }


    function CoreClass($rootScope, $scope, $mdSidenav, $mdDialog, $timeout,
